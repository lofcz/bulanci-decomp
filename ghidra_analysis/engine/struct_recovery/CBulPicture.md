...
| `+0x68` | 4 | `CDSStaticDrawableFace *` | `pBitmap` | `Create` clears; ctor `CheckedVirtualBaseCast(..., DAT_004b826c)` → drawable cast pointer; extent `origin + pBitmap->nBlitExtentW/H` @ cast `+4/+8`; `DrawSurface` → `BlitDispatch(..., pBitmap, ...)` |
...
Applied via `create_struct` (1136 bytes). Header `+0x08..+0x1F` matches `CDSChained` (`CDSChained.md`); round-3 task 33 renamed `dwField_08` / `dwField_0c` / `dwField_1c` / `wPad_16` in Ghidra. `+0x30..+0x3C` = CDSView on-screen render rect (`nRenderLeft`..`nRenderBottom`); **`pBitmap` typed `CDSStaticDrawableFace *` @ `+0x68`** (R3 todo 5: cast + `this->pBitmap`; R4 todo 5, 2026-05-30): extent disasm `cast+4/+8` → Ghidra `nBlitExtentW` / `nBlitExtentH`; cast adjustors — `CDSFlxFile+0x08` → `nBitmapHeight`/`nBitmapWidth`, heap `CDSBitmap+0x1c` → `nBbox_left`/`nBbox_top`. `palette` @ `+0x6C`, `bTeamId` / `bChromaKeyIndex` @ `+0x46C`/`+0x46D`. See `CDSFlxFile.md` downstream chain.
...
**Sequence timing (R3 todo 32 / R4 todo 32):** `CBulPicture_Create@0x0040eb30` does **not** write `seq[0x10]` (alloc `0x470` is the view shell only). Track entries hold `CDSFlxFile*` meta face `resource+4` (`AddTrackSource` / `DAT_004b8370`). `TM_AdvanceFrame` reads `seq[0x10]` / `seq[0x14]` from the bound FLX meta object after `CDSFlxFile_BindStream@0x00432ac0` copies file `+0x1c` `inMemSize` → `nSeqTotalDurationMs` and `+0x20` `flags` → `nSeqFrameCountMinusOne`. Master pack: **only 1/130** sprites have `inMemSize == sizeof(CBulPicture)`; typical values are per-clip total **ms** (210..8591). Do **not** alias `CBulPicture.wViewFlags` @ `+0x14`.
...
- ~~`inMemSize` vs `sizeof(CBulPicture)` coincidence~~ — **closed (R4 todo 32):** pack audit shows `inMemSize == 1136` on **1/130** sprites; field is per-clip total ms, not the view alloc size ([round4_task_32_report.md](./round4_task_32_report.md)).
