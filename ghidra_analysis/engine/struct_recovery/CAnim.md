...
| `+0x0C` | 4 | `uint` | `dwChainField_0c` | `CDSChained` mirror; **read** `CMina::FUN_00419fd0@0x00419fd0` when embedded |
...
| `+0x28` | 4 | `int` | `nSpatial_bucket_x` | no ctor write; `CMina_RegisterDangerZone@0x0041c0d0` (updatedItem-relative) reads with `nOrigin` for danger rect |
| `+0x2C` | 4 | `int` | `nSpatial_bucket_y` | summed in `CGameView_GetSpatialBucketKey@0x00416f50` with `dwView_aux_6c` |
...
| `+0x40` | 4 | `uint` | `dwView_pad_40` | *(Ghidra pad; `CMina_RegisterDangerZone` reads via updatedItem−0xd0)* |
...
| `+0x64` | 4 | `CBulanek *` | `overlap_entity` | Same slot as `CDSChained::dwField_64`. **Writer:** `CDSChained_ctor` `param_1[0x19]=0` @ `0x00403366`; `CDSChained_InitWithRect` @ `0x0040b611`; `CBulanek_UpdateStateFromParams` `MOV [ECX+0x64],EDX` @ `0x004178f7`. **Consumer:** `CTeleportPoint_OnEvent@0x0041fed0` @ `0x0041feff`. Owner CBulanek* for teleports: `gaming_host` @ `+0x84` via `CBulanek_AddEntity@0x0041a390` (`0x0041a3a6`) |
...
**Agent todo 22 (2026-05-30):** Renamed `pLinked_bulanek` → `overlap_entity` (`CBulanek *`); documented ctor-chain zero @ `CDSChained_ctor+0x64` and `CBulanek_AddEntity` owner @ `+0x84` (`0x0041a3a6`). R3: `set_function_this_type` on gate/AddEntity handlers; report [round3_task_22_report.md](./round3_task_22_report.md). `save_program bulanci.exe`.
...
- `dwChainField_08` / `dwChainField_0c` — chain-header semantics beyond layout mirror and CMina gate read @ `+0x0c`.
- `dwView_pad_40`, `pChain_pad_48` — no ctor write; `nSpatial_bucket_*` used by spatial key / CMina danger zone only.
