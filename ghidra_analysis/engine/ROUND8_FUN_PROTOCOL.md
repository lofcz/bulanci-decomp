# Round 8 — Single `FUN_*` rename / logic recovery
...
2. Load task **N** from `ghidra_analysis/engine/agent_todos_50_fun_r8.json` (`id` == N).
...
- Report: `ghidra_analysis/engine/fun_recovery/round8_fun_task_{NN}_report.md`
- Append one line to `ghidra_analysis/engine/agent_todos_50_fun_r8_results.jsonl`

Report sections: Task | Status | Function (single row table) | Ghidra deltas | Frida | Remaining UNK

## Prior hints

1. Check `prior_hint` in the manifest (often R7 PARTIAL carry-over).
2. Read matching `round7_fun_task_*_report.md` when the VA was analyzed in R7.
3. Re-verify in Ghidra — hints may be outdated.
