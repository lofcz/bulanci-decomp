...
| **xref_count** | 2 |
| **prior** | R6 logic task 48 slice: merged-upsampler row-buffer alloc; exact IJG `jdmerge.c` symbol UNK |

## Status

**PARTIAL** — Role, offsets, and caller contract proven via live Ghidra MCP (disasm + xrefs + decompile). No unique upstream IJG export name in repo; **`FUN_00467300` kept**. Prototype/comment applied; decompiler still shows `unaff_ESI` (register-arg convention not fully modeled).

## Function

| Address | Ghidra (before) | Ghidra (after) | Role | Evidence |
|---------|-----------------|----------------|------|----------|
| `0x00467300` | `FUN_00467300` | `FUN_00467300` | **IJG libjpeg-6b merged/fancy upsampler helper:** for each `num_components`, calls `cinfo->mem->alloc_small(cinfo, 1, 2*cinfo_field_5c+4)` and stores pointers in `upsample+0x44[]` | Disasm @ entry uses **ESI=cinfo**; loop `cmp [esi+0x64]`; `lea ebx,[upsample+0x44]`; `call [mem+4]` with `push esi` |

### Behavior (proven)

| Step | Action |
|------|--------|
| 1 | `upsample = *(cinfo + 0x1a8)` (`param_1[0x6a]` in callers) |
| 2 | `row_bytes = 2 * *(cinfo + 0x5c) + 4` (disasm: `lea ebp,[ebp+ebp+4]`) |
...
Callers **must** leave `jpeg_decompress_struct *` in **ESI** (not stack `__cdecl`); both call sites satisfy this.

### `cinfo+0x4c == 2` gate (fancy / 2v merged path)

| Caller | Site | Condition |
|--------|------|-----------|
| `jinit_merged_upsampler` | `0x004674ef` | After `gen_codes`; `cmp [esi+0x4c], 2` |
...
00467327: PUSH 1                  ; JPOOL_IMAGE/permanent pool id
...
| From | Context |
|------|---------|
| `0x004674ef` | `jinit_merged_upsampler` — post-`zlib::gen_codes`, when `[cinfo+0x4c]==2` |
| `0x0046739a` | `FUN_00467340` — fancy path setup when first `upsample+0x44` slot is null |

### Decompile (post-prototype; ESI artifact)
...
|--------|--------|--------|
| `set_function_prototype` | `0x00467300` → `void FUN_00467300(int *cinfo)` | OK |
| `set_decompiler_comment` | Entry | IJG role + callers + UNK symbol note |
| `set_variable_storage` | `cinfo` → `ESI:4` | **Blocked** — plugin requires manual decompiler edit |
| `force_decompile` | `0x00467300` | OK — `unaff_ESI` persists |
| `save_program` | `bulanci.exe` | OK |

**Not applied:** `rename_function_by_address` — no COFF/upstream name for this 0x40 B helper (R6 task 48 UNK stands).
...
| Decompiler `cinfo` in ESI vs `unaff_ESI` | **Cosmetic** — manual storage or script (`set_variable_storage` limitation) |
| `FUN_00467340` upstream name (`start_pass_merged_upsample` family) | **Out of scope** — task 35+ band |
