...
sys.path.append(str(Path(__file__).parent.parent.parent / "tools" / "bulanci_unpack"))
...
def transpile_script(raw_bytes, kind="level_script"):
    ...
    def gen_command(off, var_count):
        ...
        if spec == "_switch":
            ...
            if op == 24: # Switch
                ...
                for val, lbl in cases:
                    case_lines.append(f"if _sw == {val} then goto lbl_{lbl} end")
                return end, "\n".join(case_lines)
            else: # Select
                case_lines = []
        ...
        if op == 0: # IntConst
            ...
            return end + 4, f"if {a} {op_map[op]} {b} then goto lbl_{target:04x} end"
...
if __name__ == "__main__":
    ...
    unpacked_dir = repo_root / "unpacked"
    out_dir = repo_root / "open_bulanci" / "assets" / "levels"
