...
### Live pipeline (Ghidra GUI open, MCP attached)

When the Ghidra GUI is up and `ghidra-mcp` is serving on `:8089`, run
the lighter five-step loop instead of the full `configure.py` refresh.
This is the path to use whenever you're iterating on renames /
prototypes from the MCP side:

```
edit in Ghidra
  → python scripts/internal/export_mapping_via_mcp.py    # pulls live mapping.csv via MCP
  → python scripts/configure.py --skip-ghidra            # rescaffold + sync_units, no COFF refresh
  → python scripts/internal/rename_matched_bodies.py     # repoint matched-body signature + call sites
  → tools\ninja\ninja.exe                                # build & re-diff
```
...
If you only renamed/promoted (no prototype changes), `--skip-ghidra`
is enough. If you changed a signature, also rerun
`scripts/internal/export_mapping_via_mcp.py` first so the new types
land in `config/bulanci/mapping.csv` before sync.
...
| `scripts/internal/rename_matched_bodies.py` | Repoint definition-line names and `FUN_HHHHHHHH` call sites inside matched bodies after a Ghidra rename |
| `scripts/internal/export_mapping_via_mcp.py` | Pull a fresh `mapping.csv` from a running Ghidra MCP (no GUI shutdown needed) |
